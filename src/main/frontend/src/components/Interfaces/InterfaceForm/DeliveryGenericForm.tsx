...
const DeliveryGenericForm: React.FC<InterfaceFormProps> = ({
  ...
  const [isSubmitting, setIsSubmitting] = useState(false);
    const [excludedFields, setExcludedFields] = useState<string[]>(
        genericDeliveryConfig?.exclude_fields
            ? genericDeliveryConfig.exclude_fields.split(",").map(field => field.trim())
            : []
    );
  ...
  const authInputs: { [key in AuthType]: JSX.Element } = {
    ...
    const excludeFields = useMemo(() => {
        const schemaObj = sourceTopic?.schema ? JSON.parse(sourceTopic.schema) : null;

        const fieldPaths = schemaObj
            ? extractPathsFromSchema(schemaObj)
            : [];

        return [
            { label: 'esbDocid', value: 'esbDocid' },
            { label: '_env', value: '_env' },
            ...fieldPaths.map(path => ({ label: path, value: path }))
        ];
    }, [sourceTopic?.schema]);
