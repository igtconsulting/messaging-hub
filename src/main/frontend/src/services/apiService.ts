...
export const getServerConnections = async (connectionType: string) => {
  const response = await axios.get(
      `${import.meta.env.VITE_API_URL}/serverConnections/${connectionType}`,
      metadata
  );
  return response.data;
};

export const getMessagingHubUsers = async () => {
  const response = await axios.get(
      `${import.meta.env.VITE_API_URL}/messagingHubUsers`,
      metadata
  );
  return response.data.messagingHubUsers;
};

export const getCustomServices = async () => {
  const response = await axios.get(
      `${import.meta.env.VITE_API_URL}/customServices`,
      metadata
  );
  return response.data.customServices;
};
