...
const InterfaceDetail = () => {
  ...
  const openLastMessage = async (
    ...
    try {
      ...
      // Check if it's a 404 or empty response indicating no messages
      if (e.response?.status === 404 || !e.response?.data?.pipelineJSONString) {
        addAlert("No message for given interface found.", "error");
      } else {
    ...
    <>
      ...
      >
        <div className="max-h-[70vh] overflow-auto pr-4">
          {!!lastMessageTreeData && <SchemeTree treeData={lastMessageTreeData} />}
        </div>
      </Modal>
