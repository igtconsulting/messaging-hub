...
const ConnectionDetail = () => {
  ...
  useEffect(() => {
    if (name && !errorOccurred) {
      // Load connection info first (fast)
      const fetchConnectionData = async () => {
        try {
          ...
          addAlert("Failed to load connection info. Please try again later.", "error");
      ...
      // Load topics separately (can be slower)
      const fetchTopicsData = async () => {
        try {
          const topics = await getTopicsForConnection(name);
          if (topics) {
      ...
      // Load health data separately (can be slowest)
      const fetchHealthData = async () => {
        try {
          ...
          addAlert(
            "Failed to load connection health details. Please try again later.",
            "error"
          );
